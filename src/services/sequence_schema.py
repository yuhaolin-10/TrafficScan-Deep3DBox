...
def _drop_obsolete_scene_config_tables(cursor) -> None:
    # Scene configuration is persisted as JSON sidecar files at runtime.
    cursor.execute("DROP TABLE IF EXISTS scene_regions")
    cursor.execute("DROP TABLE IF EXISTS count_rules")
    cursor.execute("DROP TABLE IF EXISTS scene_profiles")


def ensure_sequence_schema(conn) -> None:
    cursor = conn.cursor()
    _drop_obsolete_scene_config_tables(cursor)
    cursor.execute(
        """
