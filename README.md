...
![系统初始界面](Docs/assets/readme/main-window.png)

系统提供素材导入、任务控制、图像/视频预览、区域规则配置和检测结果查看等功能，支持对图片和视频素材进行批量分析。

### 区域识别与规则配置

![自动识别应急车道和导流线](Docs/assets/readme/lane-segmentation.png)

系统可自动识别应急车道、导流线等重点监管区域，也支持手动绘制多边形区域，并为不同区域绑定占用应急车道、违法停车、逆行等规则。

![区域规则绑定](Docs/assets/readme/rule-binding.png)

### 检测结果

![视频违章检测结果](Docs/assets/readme/video-violation-result.png)

视频分析流程会结合车辆检测、单目 3D 框、目标轨迹和区域规则，输出逆行、违停等违章事件，并在右侧面板汇总检测结果。

![应急车道占用检测](Docs/assets/readme/emergency-lane-result.png)

图片分析流程会对单帧交通图像进行车辆检测、区域匹配和违章判定，绿色框表示正常车辆，红色框表示触发违章规则的车辆。
